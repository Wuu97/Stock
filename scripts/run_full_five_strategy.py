...
def main():
 p=argparse.ArgumentParser();p.add_argument('--db',required=True);p.add_argument('--profile-id',required=True);p.add_argument('--profile-version',required=True);p.add_argument('--benchmark',required=True);p.add_argument('--dataset',required=True);p.add_argument('--dataset-manifest',required=True);p.add_argument('--schedule',required=True);p.add_argument('--predictions',required=True);p.add_argument('--prediction-manifest',required=True);p.add_argument('--output-dir',required=True);a=p.parse_args()
 ...
 with writer_connection(a.db,transaction=False) as c:
  profile=json.loads(c.execute('select profile_json from competition_profiles where competition_profile_id=? and competition_profile_version=?',[a.profile_id,a.profile_version]).fetchone()[0]); start,end=map(date.fromisoformat,profile['market_data_binding']['date_range']); bench=[BenchmarkClose(date.fromisoformat(r['trade_date']),Decimal(r['adj_close'])) for r in csv.DictReader(open(a.benchmark)) if start<=date.fromisoformat(r['trade_date'])<=end]
