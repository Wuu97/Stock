...
def _existing_snapshot(connection, trade_date: date):
...
def main() -> None:
    ...
    try:
        ...
        for trade_date in days:
            existing = _existing_snapshot(connection, trade_date)
            ...
            bars, caps, evidence = canonical_daily_evidence(
                trade_date, daily, basic, factors, limits, Decimal(args.minimum_execution_market_cap),
            )
            ...
            if existing:
                ...
                if old_hash == raw_hash:
                    skipped += 1
                    continue
                conflict = directory / "conflicts" / f"{SOURCE}_{trade_date:%Y%m%d}_{raw_hash[:20]}.raw.json"
                conflict.parent.mkdir(parents=True, exist_ok=True)
                if not conflict.exists():
                    conflict.write_bytes(raw_bytes)
                raise RuntimeError(f"EVIDENCE_CONFLICT: provider response changed for {SOURCE} {trade_date}; candidate preserved at {conflict}")
            snapshot_id, cap_id = f"oos2020_tushare_market_{trade_date:%Y%m%d}", f"oos2020_tushare_cap_{trade_date:%Y%m%d}"
            ...
            try:
                snapshots.register_market_snapshot(snapshot_id, trade_date, SOURCE, daily_market_close_timestamp(trade_date), now,
                                                   str(manifest_path), manifest_hash, now)
                market_data.store_bars(snapshot_id, bars)
                universes.store_market_caps(cap_id, now, SOURCE, caps, now)
        ...
        connection.close()
    print(json.dumps({"source": SOURCE, "completed_dates": completed, "skipped_dates": skipped,
