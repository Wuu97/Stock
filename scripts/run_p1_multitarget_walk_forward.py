...
def main():
    ...
    report.update({"created_at": datetime.now(timezone.utc).isoformat(), "lineage": {"dataset": str(dataset), "dataset_sha256": dataset_hash, "dataset_manifest": str(manifest_path), "dataset_manifest_sha256": sha256(manifest_path.read_bytes()).hexdigest(), "label_schema_version": manifest["label_schema_version"], "model_family": "ridge_regression_plus_linear_logistic_v1", "parameters": parameters, "parameters_sha256": sha256(json.dumps(parameters, sort_keys=True).encode()).hexdigest()}, "scope_note": "RESEARCH only. Prediction diagnostics are not trade simulation; no fees, limits, T+1, NAV, or account promotion is implied."})
