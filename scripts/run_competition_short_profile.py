...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--db',required=True); p.add_argument('--profile-id',required=True); p.add_argument('--profile-version',required=True); p.add_argument('--strategy',required=True); p.add_argument('--account-id',required=True); p.add_argument('--benchmark-csv',required=True); p.add_argument('--schedule'); p.add_argument('--dataset'); p.add_argument('--dataset-manifest'); p.add_argument('--output',required=True); a=p.parse_args()
 with writer_connection(a.db,transaction=False) as c:
  ...
  kw={}
  if a.schedule:
   if not a.dataset or not a.dataset_manifest: raise ValueError('Ridge schedule requires frozen dataset and manifest')
   from hashlib import sha256
   raw=json.loads(Path(a.schedule).read_text()); digest=lambda path:sha256(Path(path).read_bytes()).hexdigest()
   expected={'id':a.profile_id,'version':a.profile_version,'hash':c.execute('select profile_sha256 from competition_profiles where competition_profile_id=? and competition_profile_version=?',[a.profile_id,a.profile_version]).fetchone()[0]}
   if raw.get('dataset_sha256')!=digest(a.dataset) or raw.get('dataset_manifest_sha256')!=digest(a.dataset_manifest) or raw.get('competition_profile')!=expected: raise ValueError('Ridge frozen binding mismatch')
