...
def evaluate_tracks(connection, run_ids: Sequence[str], bars: Iterable[DayBar], benchmark_ticker: str,
                    fee: FeeModel, evaluation_version: str, shares: int = 100,
                    open_gap_policy: Optional[OpenGapPolicy] = None) -> Mapping[str, dict]:
    ...
    rows = connection.execute(
        "SELECT i.item_id, i.ticker, r.target_trade_date, i.ref_close_unadj, COALESCE(m.execution_mode, 'PRODUCTION') "
    ...
    for item_id, ticker, target_date, reference_close, mode in rows:
        ...
        result, execution, reason = _evaluate_item(item_id, ticker, target_date, by_key.get((ticker, target_date)),
                                                    by_ticker.get(ticker, []), by_ticker.get(benchmark_ticker, []),
                                                    execution_rules(connection, ticker).fee(fee), shares,
                                                    None if reference_close is None else Decimal(str(reference_close)), open_gap_policy)
...
def _evaluate_item(item_id: str, ticker: str, trade_date, entry_bar: Optional[DayBar], stock_bars: Sequence[DayBar],
                   benchmark: Sequence[DayBar], fee: FeeModel, shares: int, reference_close: Optional[Decimal],
                   open_gap_policy: Optional[OpenGapPolicy]):
    if entry_bar is None:
        return EvaluationResult({}, {}, None), None, "DATA_MISSING_BAR"
    execution = match_next_open(OrderIntent("evaluation-" + item_id, "evaluation", ticker, trade_date, "BUY", shares, item_id),
                                entry_bar, fee, reference_close, open_gap_policy)
